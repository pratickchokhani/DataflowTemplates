...
public class SqlServerDialectAdapter implements DialectAdapter {
  ...
  public boolean checkForTimeout(SQLException exception) {
    // SQL Server does not support a query-level execution timeout hint in getCountQuery() (SELECT
    // COUNT(*) FROM ...),
    // so query timeouts rely on JDBC's setQueryTimeout(), which throws SQLTimeoutException.
    return false;
  }
