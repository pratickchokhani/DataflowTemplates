...
public class SqlServerDialectAdapterTest {
  ...
  public void testDiscoverTableSchemaSuccess() throws Exception {
    ...
    // 5 rows covering: charMaxLen, numPrecision+numScale, numPrecision only, neither, bit
    when(mockResultSet.next()).thenReturn(true, true, true, true, true, false);
    when(mockResultSet.getString("TABLE_NAME"))
        .thenReturn("table1", "table1", "table1", "table1", "table1");
    when(mockResultSet.getString("COLUMN_NAME"))
        .thenReturn("col_varchar", "col_decimal", "col_int", "col_datetime", "col_bit");
    when(mockResultSet.getString("DATA_TYPE"))
        .thenReturn("varchar", "decimal", "int", "datetime", "bit");

    when(mockResultSet.getLong("CHARACTER_MAXIMUM_LENGTH")).thenReturn(100L, 0L, 0L, 0L, 0L);
    when(mockResultSet.getLong("NUMERIC_PRECISION")).thenReturn(0L, 10L, 10L, 0L, 0L);
    when(mockResultSet.getLong("NUMERIC_SCALE")).thenReturn(0L, 2L, 0L, 0L, 0L);

    // Sequence of rs.wasNull() calls across the 5 rows:
    ...
    // Row 5 (bit): charMaxLen->true, numPrecision->true, numScale->true
    ...
    assertEquals("bit", cols.get("col_bit").getName());

    // Verify BIT column discovered in discoverTableSchema triggers custom CAST(... AS BIGINT) in
    // getBoundaryQuery
    String boundaryQueryBit =
        adapter.getBoundaryQuery("table1", ImmutableList.of("col_varchar"), "col_bit");
  ...
  public void testDiscoverTableIndexesAllTypesAndBitBoundary() throws Exception {
    ...
    when(mockResultSet.getLong("cardinality")).thenReturn(500L);
    when(mockResultSet.getInt("max_length")).thenReturn(200);
    when(mockResultSet.getInt("scale")).thenReturn(6);
    when(mockResultSet.getString("collation_name")).thenReturn("SQL_Latin1_General_CP1_CI_AS");
    ...
    assertEquals(500L, infoList.get(0).cardinality());
    assertEquals(Integer.valueOf(200), infoList.get(0).stringMaxLength());
    assertEquals(
        "SQL_Latin1_General_CP1_CI_AS", infoList.get(0).collationReference().dbCollation());
    assertEquals(IndexType.NUMERIC, infoList.get(1).indexType());
    assertEquals(IndexType.DECIMAL, infoList.get(2).indexType());
    assertEquals(Integer.valueOf(6), infoList.get(2).numericScale());
