...
/**
 * Integration test for {@link SpannerToSourceDb} Flex template for SQL Server data types targeting
 * PostgreSQL dialect Spanner.
 */
...
public class SpannerToSQLServerDataTypesPGDialectIT extends SpannerToSourceDbITBase {
  ...
  private static final String SPANNER_DDL_RESOURCE =
      "sqlserver/SpannerToSQLServerDataTypesIT/pg-dialect-spanner-schema.sql";
  ...
  private Map<String, List<Value>> getSpannerTableData() {
    ...
    spannerRowData.put(
        "decimal",
        List.of(
            Value.pgNumeric("68.7500"),
            Value.pgNumeric("99999999999999.9999"),
    ...
    spannerRowData.put(
        "money",
        List.of(
            Value.pgNumeric("-922337203685477.5808"),
            Value.pgNumeric("922337203685477.5807"),
            Value.pgNumeric(null)));
    spannerRowData.put(
        "smallmoney",
        List.of(
            Value.pgNumeric("-214748.3648"),
            Value.pgNumeric("214748.3647"),
            Value.pgNumeric(null)));
    spannerRowData.put(
        "float",
        List.of(
    ...
    spannerRowData.put(
        "json_to_varchar", List.of(Value.pgJsonb("{\"a\":1,\"b\":\"test\"}"), Value.pgJsonb(null)));
    spannerRowData.put(
        "array_to_varchar",
        List.of(
            Value.float64Array(new double[] {1.5, 2.5, 3.5}), Value.float64Array((double[]) null)));
  ...
  private Map<String, List<Map<String, Object>>> getExpectedData() {
    ...
    expectedData.put(
        "json_to_varchar", createRows("json_to_varchar", "{\"a\": 1, \"b\": \"test\"}", null));
    expectedData.put("array_to_varchar", createRows("array_to_varchar", "[1.5,2.5,3.5]", null));
